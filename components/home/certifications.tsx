                            ...
                            return (
                                <motion.div
                                    ...
                                    className={`absolute cursor-pointer rounded-xl border border-zinc-800 bg-zinc-900 overflow-hidden flex flex-col items-center justify-center 
                                        ${isCenter ? "z-20 shadow-[0_0_50px_rgba(0,0,0,0.5)] border-zinc-700" : "opacity-40 blur-[1px] grayscale"}`}
                                    initial={{
                                ...
                                >
                                    <div className="w-64 h-40 bg-zinc-950 flex items-center justify-center relative group/cert">
                                        {isCenter && (
                                    ...
                                    <div className="w-full flex items-center justify-center p-3 bg-zinc-900 border-t border-zinc-800">
                                        {
                                            item.licensorLogo && !item.licensorLogo.includes("....") ? (
                                                <img src={item.licensorLogo} alt={item.licensor} className="h-6 w-6 object-cover rounded-full" />
                                            ) : (
                                                <p className="text-xs font-bold text-zinc-300 truncate text-center px-2">{item.licensor}</p>
                                            )
                                        }
                                        <p className="text-xs font-bold text-zinc-300 truncate text-center px-2">{item.name}</p>
                                    </div>
                                </motion.div>
                            );
                        })}
